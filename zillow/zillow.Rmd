...
boxplot(logerror ~ addNA(storytypeid), set, main = "storytypeid") 
#수영장 관련 
...
plot(bathroomcnt~logerror,set) #피라미드꼴.. (결측x)
plot(bedroomcnt~logerror,set) #피라미드꼴 (결측x)
plot(roomcnt~logerror,set) #결측x, 방 0개인것만 에러가 다양함, 나머지는 비슷 

#결측값에 대한 고민을 해야함
plot(basementsqft~logerror,set) #변동성 적음 (90232)
plot(calculatedbathnbr~logerror, tr) #피라미드꼴 (1182)
plot(finishedfloor1squarefeet~logerror,set) # 거의 1자임 (물방울?)- (83419) 
...
plot(finishedsquarefeet6~logerror,set) #Base unfinished and finished area (89854) #대부분 결측, 거의 1자
plot(fireplacecnt~logerror,set) # 대부분결측(80668) #피라미드 구조 
plot(fullbathcnt~logerror,set) # 피라미드 구조 (1182)
...
plot(taxdelinquencyyear~logerror,set)#역피라미드꼴(88492) (6~15, 99, 결측)
```
